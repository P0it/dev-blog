...
**문제** 일정은 카톡에, 사진은 각자 갤러리에, 가보고 싶은 곳은 인스타 저장함에 흩어져 있었어요. 둘이 같이 쓸 곳이 없었습니다
**타겟** 저와 연인 둘. 하루에 몇 번씩 짧게 들어와요
**해법** "내 것을 공유한다"가 아니라 처음부터 둘의 공간에 넣습니다. 커플을 최상위에 두고, 누가 넣든 상대 폰에 곧바로 뜨게 했어요
**범위** 흩어져 있던 걸 한 공간에 모으는 데까지예요. 일정과 기록에 스토리·피드, 하루 한 번 참여하는 이벤트까지만 담았습니다
**범위** 수익화는 안 붙였어요. 사용량 게이트는 가장 열심히 쓰는 소수에게만 청구서를 보내면서 총액은 얼마 안 되는 구조라 접었어요
**범위** 실시간 동시 대결은 넣지 않았어요. 접속 타이밍이 안 맞으면 죽는 기능이라 비동기 점수 승부로 충분했어요
**우선순위** 일정 → 데이트 앨범 → 피드 → 홈 → 스토리 순으로 세웠어요
