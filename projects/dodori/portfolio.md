...
**설명** 오늘의 주제·추천곡·게임이 하루치로 깔립니다

### 캘린더

**파일** ./screens/02-calendar.png
**설명** 둘의 일정과 기념일을 한 달로 봅니다

### 라이브러리

**파일** ./screens/03-playlist.png
**설명** 가보고 싶은 곳을 쟁여두는 자리입니다

### 데이트 앨범

**파일** ./screens/05-track.png
**설명** 다녀온 하루가 사진·코스가 붙은 한 장으로 남습니다

### 피드

**파일** ./screens/04-feed.png
**설명** 게시물과 24시간 스토리가 쌓입니다

## 데이터와 API

### 네이버 지역검색 API

**용도** 장소를 검색해 후보를 띄우고, 담은 곳만 우리 쪽에 저장한다
**방식** REST · 검색할 때만
**링크** https://developers.naver.com/docs/serviceapi/search/local/local.md
**주의** 하루 25,000건 제한. 키를 앱에 두지 않으려고 서버가 대신 부른다. 돌아오는 좌표가 지도와 다른 체계라 한 번 변환해서 쓴다

### 한국천문연구원 특일정보

**용도** 임시공휴일과 선거일을 받아 달력에 빨간 날로 세운다
**방식** REST · 월 1회
**갱신** 자동
**링크** https://www.data.go.kr/data/15012690/openapi.do
**주의** 매년 같은 규칙으로 도는 공휴일은 앱이 직접 계산한다. 그래서 이 키가 없어도 달력은 멀쩡하다

### iTunes Search API

**용도** 추천 곡의 앨범 이미지와 30초 미리듣기를 붙이고, 실제로 있는 곡인지 확인한다
**방식** REST · 시드 만들 때만
**아이콘** Apple
**링크** https://performance-partners.apple.com/search-api
**주의** 비슷한 이름에도 아무 결과나 돌려줘서, 가수와 제목이 둘 다 맞을 때만 채택한다. 이 검증이 지어낸 곡을 걸러내는 장치가 됐다

### 네이버 지도

**용도** 담은 장소와 하루 동선을 지도 위에 그린다
**방식** 앱 SDK · 웹 JS
**아이콘** Naver
**링크** https://www.ncloud.com/product/applicationService/maps
**주의** 콘솔에 등록한 주소와 어긋나면 인증 실패가 에러가 아니라 빈 화면으로 나온다

### 카카오 로그인

**용도** 따로 가입시키지 않고 카카오 계정으로 바로 들어오게 한다
**방식** 앱 SDK · 웹 OAuth
**아이콘** Kakao
**링크** https://developers.kakao.com/docs/latest/ko/kakaologin/common
**주의** 앱은 개발용 빌드가 따로 있어야 하고, 안드로이드는 첫 빌드 뒤 키 해시를 콘솔에 등록해야 한다

## 기획

**문제** 각자 약속이 생기면 카톡으로 알려 주는데, 그걸 그냥 기억하거나 제 캘린더에 옮겨 적거나 상대가 한 달치 캘린더를 캡처해 보내야 했어요. 어느 쪽도 불편했습니다
**타겟** 저와 연인 둘. 하루에 몇 번씩 짧게 들어와요
**해법** 커플에게 꼭 필요한 기능만 골라 뽑아 서로의 일정부터 세웁니다. "내 것을 공유한다"가 아니라 처음부터 둘의 공간에 들어가, 누가 넣든 상대 폰에 곧바로 떠요
**범위** 일정과 기록에 스토리·피드, 하루 한 번 참여하는 이벤트까지예요. 수익화와 실시간 대결은 넣지 않았습니다

## 구상

- 일정을 둘이 같은 캘린더에서 잡는다
- 다녀온 하루가 사진·코스가 붙은 앨범 한 장으로 남는다
- 가보고 싶은 곳을 쟁여두고 다음 데이트 코스로 꺼내 쓴다
- 사진을 둘만 보는 곳에 올린다
- 앱스토어·플레이스토어에 올려서 우리 말고도 쓴다

## 인터뷰

**소개** 커플 둘이 캘린더·사진첩·기록을 한 공간에 같이 쌓는 앱이에요. "내 것을 상대에게 공유한다"가 아니라 처음부터 둘의 공간에 들어가서, 누가 넣든 상대 폰에 곧바로 뜹니다
**소개** 홈에는 오늘 것 세 개(주제·추천곡·게임)가 있고, 셋 다 내가 먼저 해야 상대 답이 열려요. 다녀온 하루는 사진과 코스가 붙은 앨범 한 장으로 남습니다
...
시작은 일정이었어요. 둘이 같이 보는 캘린더 하나를 먼저 세우고, 커플에게 꼭 필요한
기능만 골라 뽑아 붙이기로 했습니다. 일정을 붙이고 나니 데이트 계획을 짜는 자리가
자연스레 따라왔고, 다녀온 하루를 남기는 아카이빙도 필요해졌어요. 매일 들어올 이유는
그다음 문제였는데, 오늘의 주제·게임·스토리가 거기서 나왔습니다.

### 2. "공유"가 아니라 처음부터 둘의 것으로
...
### NativeWind가 버튼 스타일을 버렸어요 — 카카오 버튼이 배경 없이 렌더링
...
**결론** 원인은 NativeWind v4였어요. `jsxImportSource: 'nativewind'` 가 **모든 JSX**를 css-interop으로 감싸는데, 그 과정에서 `Pressable` 의 함수형 `style`(`({pressed}) => …`)이 그대로 버려졌습니다. 에러도 경고도 없었어요. 그런데 정작 `className` 사용처가 프로젝트에 0개였습니다 — 쓰지도 않는 라이브러리가 렌더링을 망가뜨리고 있던 거예요. NativeWind를 제거하고 RN `style` + 토큰으로 통일했고, 덤으로 Metro 패치 우회도 필요 없어졌습니다.
...
### cron이 몇 달간 401로 실패하고 있었어요 — 어디에도 표시가 안 나던 실패

**증상** 매일 도는 `daily-release` 와 월 1회 `sync-holidays` 가 실행은 되는데 아무 일도 일어나지 않았어요.

**시도** 함수 코드와 cron 등록을 먼저 확인했습니다. 둘 다 정상이었어요.

**결론** Vault에 저장된 service role 키 자리에 **문서의 자리표시자 문자열이 그대로 들어가 있었습니다.** 설정 안내를 복사해 실행하면서 `<SERVICE_ROLE_KEY>` 를 실제 키로 치환하지 않은 거예요. cron은 401을 받고 그냥 건너뛰었고, 실패가 화면에 드러나지 않는 기능이라 몇 달간 몰랐습니다. 지금은 배포 후 수동 설정 항목을 CLAUDE.md에 경고와 함께 적어 두고, 미설정일 때 무엇이 안 되는지도 같이 써 뒀어요.

## 남은 것

- 동영상은 아직 올릴 수 없어요. 사진만 주고받는 상태라, 기록이 길어질수록 아쉬운 자리입니다.
- 알림이 앱을 켜 둔 동안에만 와요. 앱 밖에서도 받게 하려면 푸시를 마저 붙여야 합니다.
- 계정을 지우는 자리가 없어요. 떠나는 사람이 자기 기록을 정리할 방법이 있어야 합니다.
