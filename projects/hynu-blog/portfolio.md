...
slug: hynu-blog
name: hynu.blog
tagline: 글쓰기를 AI에게 맡긴 개인 기록 블로그
year: "2026"
logo_bg: "#0E1116"
logo_file: ./logo.svg
...
**파일** ./screens/06-ai-draft.png
**설명** 링크와 메모를 넣으면 초안 작업이 큐에 걸린다
...
**용도** 링크를 읽고 규약대로 글 초안을 쓰거나, 발행된 글을 피드백대로 고친다
**방식** CLI · 로컬 워커가 호출
**갱신** 큐가 비어 있지 않을 때
**링크** https://docs.claude.com/en/docs/claude-code
**주의** 서버에서 부르지 않고 내 컴퓨터에서 부른다. 구독 계정으로 도는 CLI라 클라우드에 얹기 어렵고, 한 편을 쓰는 데 몇 분씩 걸려 웹 요청 안에서 끝나지도 않는다
...
**갈라짐** 폰이나 태블릿 → 어드민 화면 / 노트북 앞 → Claude Code 세션에 바로
...
**갈라짐** 결이 맞다 → 제목·태그만 다듬는다 / 어긋난다 → 피드백을 적어 다시 맡긴다
...
글과 프로젝트를 쓰고 고치는 화면입니다. 무거운 일은 직접 하지 않고 작업만 걸어 둡니다.

### Next.js 서버

읽는 화면을 그리고, 어드민이 누른 것을 DB와 Storage에 적습니다. AI는 부르지 않아요.

### Postgres

글·프로젝트·카테고리·조회수가 여기 있습니다. 밀린 AI 작업 줄도 같은 곳에 테이블로 놓여 있어서, 서버와 워커가 이 테이블만 보고 서로를 압니다.

### AI 워커

제 컴퓨터에서 도는 스크립트입니다. 밖에서 들어오는 접속은 받지 않고 Supabase 쪽으로 나가기만 해요.

### Claude Code

워커가 규약 문서와 요청을 통째로 넘겨 부릅니다. 결과 본문은 워커를 거쳐 DB로 들어갑니다.
