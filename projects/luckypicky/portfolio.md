...
**설명** 선행 한 번에 잎이 하나씩 붙습니다

### 선행 기록

**파일** ./screens/02-record.png
**설명** 오늘 한 일을 한 줄로 적어 넣습니다

### 클로버 완성

**파일** ./screens/03-clover.mp4
**설명** 네 장째를 채우면 클로버가 상단 배지로 날아갑니다

### 행운 뽑기

**파일** ./screens/04-gacha.png
**설명** 코인 하나로 캡슐 머신을 돌립니다

### 행운지수

**파일** ./screens/05-fortune.png
**설명** 왕복하는 게이지를 탭한 순간이 오늘 점수가 됩니다

### 행운지수 결과

**파일** ./screens/06-fortune-result.png
**설명** 잡은 점수와 오늘의 색·숫자·아이템을 한 장으로 받습니다

### 보관함

**파일** ./screens/07-dex.png
**설명** 뽑은 행운권이 등급별로 쌓입니다

### 재료 고르기

**파일** ./screens/08-forge.png
**설명** 태울 카드를 직접 골라 성공 확률을 올립니다

### 강화 연출

**파일** ./screens/09-forge.mp4
**설명** 재료가 빨려 들어가고 게이지가 한 바퀴 돌면 성패가 갈립니다

### 나의 기록

**파일** ./screens/10-archive.png
**설명** 선행한 날이 달력에 색으로 남습니다
...
**게임 규칙** 잎 적립·클로버 완성·뽑기·강화·재조합이 전부 서버 함수 안에서 일어나요. 앱은 결과만 받습니다.
**문구 배달** 홈 문구와 행운지수 문구도 서버에서 내려받아요. 비어 있거나 장애면 앱에 실린 문구로 떨어집니다.
**주의** 익명 로그인 세션은 기기에 묶여요. 기기를 바꾸면 새 계정이 돼서, 복구 코드를 따로 만들었습니다.
...
**지급 경로** 대시보드에 적는 리워드 수량은 표시용이에요. 실제 지급은 앱이 서버에 요청해서 합니다 — 클라이언트가 재화를 스스로 만들지 못하게요.
**iOS** 구글이 공식으로 안내하는 광고 네트워크 목록을 그대로 넣었고, 추적 동의 프롬프트는 첫 화면이 뜬 뒤에 띄워요.

## 인터뷰

**소개** 착한 일을 한 줄 적으면 잎이 하나 붙고, 네 장이 모이면 네잎클로버가 완성되는 앱이에요. 그 클로버로 행운권을 뽑고 모읍니다
**계기** 평소에 업보라는 걸 자주 생각했어요. 카르마라고도 하죠. 야구선수 오타니도 자기 덕을 위해 쓰레기를 줍는다고 알려져 있고요. 제가 좋아하는 롤 프로팀 T1 의 팬 문화도 비슷해요. 롤드컵 기간이 되면 팬들이 일부러 착한 일을 하고 인증을 올립니다. 내 선행이 조금이라도 응원하는 팀에 보탬이 되길 바라면서요
**계기** 그러다 생각이 들더라고요. 운이라는 게 정말 문득 찾아오기만 하는 걸까, 그렇게 수동적인 걸까. 능동적으로 얻을 수는 없을까. 그러다 네잎클로버가 떠올랐고, 선행을 쌓아 스스로 운을 만드는 서비스로 만들어 봤어요
**사용자** 2030 여성이요. 하루에 한두 번 잠깐 들어와 한 줄 적고 나가는 쪽입니다
...
- 착한 일을 한 줄 적으면 잎이 하나 붙고, 네 장이 모이면 클로버가 완성된다
- 완성된 클로버로 소원권을 만든다
- 지난 기록을 달력에서 한눈에 본다
- 한국어·영어·일본어로 같이 낸다
...
잎 적립, 클로버 완성, 뽑기, 강화, 재조합, 복구 코드 이관을 맡는다. 앱 권한으로는 여기를 거치지 않고 데이터에 손댈 수 없다.
...
앱에 같이 실려 나가는 폴백 문구. 서버 문구를 못 받은 자리에 그대로 들어간다.
...
작업 PC 가 윈도우라 화면 확인은 전부 안드로이드 에뮬레이터에서 했어요. iOS 는 코드만 맞춰 두고 빌드 검증을 못 한 채로 왔습니다.
...
언어는 한국어·영어·일본어까지만 열었어요. 중국어는 간체·번체가 갈리는 데다 본토 스토어는 심사와 퍼블리셔가 따로라, 혼자 감당할 운영비가 아니라서 뺐습니다.
...
- 웹으로 공개하면 재화 경제가 깨져요. 웹에는 광고가 없어서 보상형이 즉시 지급되거든요. 막을 방법을 먼저 만들어야 합니다.
