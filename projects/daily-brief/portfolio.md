...
**용도** 국내 종목 촉매를 여기서 잡는다. 종목 지정 없이 전체 시장 공시를 받는다
**방식** REST · 하루 1회
**갱신** 매 실행
**링크** https://opendart.fss.or.kr/api/list.json
**주의** 아침 6~7시엔 당일 공시가 거의 없어 직전 거래일까지 되짚는 룩백이 필수다. 3일치면 공시가 2,000건대로 쌓인다(실측 2,179건)

### SEC EDGAR

**용도** 미국 기업의 수시공시와 내부자 거래를 촉매로 쓴다
**방식** Atom 피드 · 최신 N건
**갱신** 매 실행
**링크** https://www.sec.gov/cgi-bin/browse-edgar
**주의** User-Agent 에 실명과 이메일을 넣어야 응답이 온다. 없으면 수집을 통째로 건너뛰게 막아뒀다

### KRX Open API

**용도** 국내 전종목 시세와 시가총액을 받아 초소형주 쏠림을 거른다
**방식** REST · 키 헤더 인증
**갱신** 거래일마다 하루치
**링크** https://openapi.krx.co.kr
**주의** 전종목 응답이 요청당 26~30초 걸려 타임아웃을 90초로 올렸다. 투자자별 순매수와 공매도 잔고는 이 API 범위 밖이라 미제공으로 확정했고, 네이버 금융 시세는 robots.txt 가 일반 UA 를 전면 차단해 배제했다

### 미국 공공 데이터

**용도** 정부 조달, 신약 승인, 의원 주식 거래를 선행 지표로 얹는다
**방식** REST · GitHub raw JSON
**갱신** 매 실행
**링크** https://api.usaspending.gov/api/v2
**아이콘** USAspending
**주의** USAspending·openFDA·Senate Stock Watcher 셋을 각각 따로 감싸 하나가 죽어도 나머지 수집이 계속된다. 의원 거래는 공식 API 가 아니라 커뮤니티가 관리하는 데이터셋이다

### 보도자료 와이어

**용도** 기업 발표 원문을 언론 필터를 거치기 전에 받는다
**방식** RSS · 와이어 3곳
**갱신** 매 실행
**주의** GlobeNewswire, PR Newswire, Business Wire 를 한 묶음으로 돌린다

### 네이버 금융 리서치

**용도** 증권사 리포트를 모아 국내 종목의 배경 자료로 쌓는다
**방식** 크롤링 · 하루 ~90건
**갱신** 매 실행
**링크** https://finance.naver.com
**아이콘** Naver
**주의** 등급 변경은 이미 주가에 반영된 늦은 신호라 국내 픽 직접 입력에서는 뺐다. 수집과 표시만 유지한다

### yfinance

**용도** 발굴 트랙 재무와 픽의 진입가·현재가를 여기서 가져온다
**방식** 라이브러리 · Yahoo Finance
**갱신** 매 실행 · 발굴은 수동
**아이콘** Yahoo
**주의** 수천 종목을 한 번에 긁으면 레이트리밋에 걸려 병렬 수를 묶고 백오프를 걸었다. 멀쩡한 종목도 자주 실패해서 "없는 티커"와 "확인 불가"가 구분되지 않는다 — 확인 실패를 탈락 근거로 쓰지 않는 이유다
...
**갱신** 매 실행
**아이콘** Anthropic
**주의** Max 플랜 할당량으로 청구된다. 환경에 API 키가 있으면 CLI 가 아무 말 없이 API 과금 모드로 넘어가서, 그 키를 절대 설정하지 않는 게 이 프로젝트의 규칙이 됐다. 응답은 DB 에 캐시해 같은 걸 두 번 부르지 않는다
...
- [ ] 인포스탁 테마주 분류를 긁어 밸류체인 매핑 DB 를 만든다

## 기획

**소개** 평일 아침 7시에 혼자 돌아가는 파이프라인이에요. 밤사이 올라온 DART 공시, SEC 8-K·Form 4, 정부 조달, FDA 승인, 의원 주식 거래 같은 걸 긁어서 중복을 걸러내고 점수를 매긴 뒤, 진짜 촉매가 붙은 종목만 카드로 남깁니다
**소개** 뉴스 리더가 아니라 그날의 종목 브리핑이에요. 왜 이 종목인지·촉매가 뭔지·확신도가 얼마인지가 카드 한 장에 들어가고, 강한 촉매가 없는 날은 억지로 채우지 않고 화면을 비웁니다
...
**넣지 않은 것** 로그인과 계정. 배포된 페이지는 누구나 읽을 수 있지만 읽기 전용이고, 개인화는 없어요
...
LLM 을 어떻게 부를지가 첫 갈림길이었어요. Anthropic Python SDK 를 쓰면 API 과금이 붙는데, 이미 Max 플랜을 구독 중이었거든요. `claude -p` CLI 를 서브프로세스로 부르면 Max 할당량으로 청구된다는 걸 확인하고 그쪽으로 갔습니다. 그 뒤로 "`ANTHROPIC_API_KEY` 를 절대 설정하지 않는다"가 이 프로젝트의 규칙이 됐어요. 그 키가 환경에 있으면 CLI 가 아무 말 없이 API 과금 모드로 넘어가거든요.

**붙인 것** DART Open API, Claude Code, SQLite, launchd
...
초기 방침은 "추천 금지, 시그널만 띄운다"였어요. 근데 실제로 읽어보니 "일반적으로 이런 공시는 긍정적으로 해석됩니다" 수준의 하나 마나 한 문장만 나오더라고요. 포털 앱에서 보는 거랑 다를 게 없었어요. 어차피 혼자 쓰는 시스템이고 외부 배포도 없으니, 방향성 있는 판단을 내놓게 방침을 뒤집었습니다. 매수·매도 방향, 촉매 강도 등급, 관련 ETF 까지 말하게 했어요.

그랬더니 바로 다음 문제가 왔어요. LLM 이 없는 종목을 지어내기 시작한 겁니다. 그럴듯한 티커에 그럴듯한 촉매를 붙여서요. 그래서 픽이 나온 뒤에 2차 검증 단계를 하나 더 뒀습니다. 이슈 단위로 "이 촉매가 근거 공시에 실제로 있나"를 먼저 확인해서 날조된 촉매는 이슈째 버리고, 남은 픽마다 종목과 공시 주체가 맞물리는지 대조해요. 티커 실존은 yfinance 와 자체 DB 로 확인하는데, 확인 실패를 곧바로 탈락시키진 않습니다 — 네트워크 한 번 튀었다고 멀쩡한 종목을 잃으면 안 되니까 검토 플래그만 달고 화면에 사유를 노출해요.
...
### 5. 촉매만 쫓다 보니 조용한 회사를 못 봤어요

파이프라인 전체가 이벤트 구동이었어요. 공시나 뉴스가 떠야만 후보 풀에 들어옵니다. 그러니까 "무언가 발생한" 종목만 잡히고, 사건은 없는데 재무가 좋은 회사는 영영 안 걸려요. 단발성 호재만 쫓고 있다는 걸 몇 주 만에 알았습니다.

그래서 촉매와 완전히 분리된 발굴 트랙을 새로 팠어요. 고정 유니버스를 잡고, 가치·퀄리티·성장 팩터를 유니버스 내 백분위로 만들어 가중 합성합니다. 이 랭킹 자체는 LLM 없이 결정론적이라 단위 테스트가 가능해요. 그 위에 LLM 심층 리서치를 얹어서 이 회사를 봐야 하는 이유와 "왜 아직 발굴되지 않았나"를 씁니다. 수백 종목 재무 수집이라 무거워서 자동 스케줄에는 안 넣고, 손으로 돌릴 때만 갱신되게 했어요.

처음 잡은 유니버스가 이미 다 아는 대형주 닫힌 집합이라 발굴이랍시고 삼성전자를 내놓던 시기가 있었는데, 그건 아래 시행착오에서 따로 풉니다. 지금은 미국 5,600개, 국내 2,600개 전체 목록을 봐요.

**붙인 것** yfinance, NASDAQ Trader, KIND

### 6. 로컬 맥에 묶인 게 결국 발목을 잡았어요

여기까지 데이터가 전부 로컬 SQLite 와 정적 파일에 있었어요. 머신을 두 대 쓰기 시작하니 어느 맥에서 생성했느냐에 따라 데이터가 갈렸고, 달력에서 지난 브리핑이 통째로 사라지는 일도 났습니다.

그래서 Supabase 를 단일 원본으로 못박았습니다. 어느 머신에서 돌리든 DB 에 upsert 하고, 로컬 파일은 DB 에서 파생된 캐시로 격하했어요. Vercel 은 빌드할 때 DB 에서 직접 끌어와 자기 정적 파일을 만듭니다. 그러니까 데이터를 git 에 커밋할 필요가 없어졌고, 파이프라인은 DB 에 쓴 다음 Deploy Hook 으로 POST 한 번만 던지면 돼요. 런타임 DB 의존이 없으니 anon key 노출이나 RLS 설계도 필요 없고요.

그다음엔 실행 자체를 옮겼습니다. 맥북 전원·절전 상태에 묶인 로컬 스케줄러로는 실행률이 63% 밖에 안 나왔거든요. GitHub Actions 로 옮기면서 Max 플랜 할당량은 그대로 유지했어요 — `claude setup-token` 으로 발급한 OAuth 토큰을 시크릿에 넣고 러너에서 CLI 를 돌립니다. `.env` 는 dotenvx 로 암호화해서 저장소에 커밋해뒀기 때문에 시크릿은 복호화 개인키 하나면 끝이에요. 주말은 아예 안 돌립니다. 장이 안 열리는 날 종목 전용 화면에 보여줄 게 없거든요.
...
**결론** 원인이 두 개 겹쳐 있었어요. 진짜 원인은 진입 기준가를 **추천일 당일 종가**로 잡은 거였습니다. 현재가도 최신 종가라서, 다음 세션이 닫히기 전까지 둘이 같은 값이 되어 첫날은 무조건 0% 가 나왔어요. 추천은 한국 아침 — 미국장 개장 전에 나오니까, 그 시점에 알 수 있는 마지막 가격인 **직전 거래일 종가**가 실제 진입가입니다. 기준가 정의를 직전 거래일 종가로 바꾸고, 주말·휴장은 추천일 직전 마지막 거래일을 취해 자동 보정했어요.
...
**결론** 구조 문제였어요. 프론트는 `frontend/public/` 정적 JSON 만 읽는데 매일 아침 정리 작업이 "오늘 것"만 남기고 로컬 브리핑 파일을 지우고 있었습니다. 픽 히스토리는 Supabase 에서 읽어서 살아남았던 거고요. 게다가 머신을 두 대 쓰기 시작하면서 어느 맥에서 생성했느냐에 따라 파일이 갈렸어요. Supabase 를 단일 원본으로 못박고, 로컬 파일은 DB 파생 캐시로 격하하고, 배포는 빌드시 DB 에서 직접 끌어오게 바꿨습니다. 보존 일수도 90일로 한 군데서만 정의하게 일원화했어요.
...
**결론** 방향을 두 번 틀었어요. 첫째로 건별 호출을 배치 처리로 바꿨습니다 — 요약과 번역을 여러 건씩 한 번에 넘기니 호출 수가 급감했어요. 둘째로 국내·해외를 순차가 아니라 병렬로 던졌습니다. 통제 측정해보니 동시 2개도 각각 ~110초로 단독 실행(~136초)과 비슷했어요. 처리량 경합이 이 규모에선 안 일어난다는 뜻이라 병렬이 순차(합 ~250초)보다 확실히 빨랐습니다. 결국 과거 타임아웃은 병렬 탓이 아니라 일시적 서버 지연과 thinking 변동성이었고, 방어책은 타임아웃 420초 한 줄로 정리했어요. 입력을 15건까지 깎았던 것도 되돌렸습니다.
...
**결론** 로그를 보니 국내 하위 티어 후보가 0 이 아닌 날이 하루도 없었어요. 그런데 LLM 에는 후보 2건만 넘어가고 있었습니다. 후보 풀 빌더가 점수 하한 75 를 전 티어에 공통 적용하는데, 오케스트레이터가 국내 뉴스에 40점을 고정 부여하고 있었어요. 한경·매경·연합이 100% 탈락하고 있었던 겁니다. 애초에 범주 오류였어요 — 1티어(공시·리서치)는 내용 기반 점수(45~95)고 2·3티어(뉴스)는 소스 신뢰도를 환산한 점수(42~65)라 척도가 다른데 하나의 하한을 씌운 거죠. 하한을 티어별로 쪼개고(국내 75/40), 국내 뉴스 점수도 소스별로 나눴어요(한경·매경·연합 50, Google News 42). 1티어 하한 75 는 그대로 둬서 정기 공시가 밀고 들어오는 건 계속 막습니다. 이 하한이 다시 무너지지 않게 회귀 테스트를 붙였어요.

### 아침 브리핑이 두 주 가까이 통째로 비었어요

**증상** 어느 날 달력을 보니 7월 중순부터 2주간 브리핑이 하나도 없었어요. 코드는 멀쩡했고 손으로 돌리면 잘 돌았습니다.

**시도** 처음엔 `launchd` 스케줄 설정을 의심했어요. 평일 아침 자동 wake 를 걸고, 절전 방지와 PATH 고정을 추가했습니다. `claude` 실행 파일을 PATH 에서 못 찾았는데 에러 하나 없이 빈 브리핑만 만들어지는 사고도 있어서, 파이프라인 시작 시점에 preflight 체크를 넣어 크게 로그를 남기게 했고요.

**결론** 고쳐도 결번이 계속 났어요. Supabase 기록을 세어보니 6~8월 평일 약 60일 중 22일이 비어 실행률이 63% 였습니다. 맥북을 닫아두거나 여행을 가면 그날은 그냥 없는 거였어요 — 로컬 스케줄러로 푸는 문제가 아니었습니다. GitHub Actions 로 옮겼어요. 이미 데이터 원본이 Supabase 이고 배포도 Deploy Hook 이라 러너가 저장소에 쓸 일이 없어서, 실행 환경만 갈아끼우는 낮은 위험의 변경이었습니다. Max 플랜은 `claude setup-token` OAuth 토큰으로 유지했고, `.env` 는 이미 dotenvx 암호화본이라 시크릿은 복호화 개인키 하나로 끝났습니다. launchd 설정은 지우지 않고 로컬 수동 실행 경로로 남겨뒀어요.
...
- 국내 촉매 소스가 DART 하나뿐이다. 해외는 EDGAR·정부 조달·FDA·와이어로 다변화됐는데 국내는 대안이 마땅치 않다
