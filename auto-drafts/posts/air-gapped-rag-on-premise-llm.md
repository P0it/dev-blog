---
title: 인터넷 없는 회사에서도 생성형 AI를 쓸 수 있을까요?
...
---
> 됩니다. 다만 8편 RAG 의 부품 — LLM, 임베딩 모델, 벡터 DB, 컨테이너 이미지, 패키지 — 다섯 가지를 전부 밖에서 받아 안으로 옮겨야 합니다. 이 반입 절차와 그 뒤의 보안 갱신이 폐쇄망 AI 운영의 핵심 업무입니다.

금융·공공·방산 쪽에서 일하시는 분들은 "우리는 인터넷이 안 되는데 ChatGPT 같은 걸 쓸 수 있나요?"라는 질문을 자주 받으실 겁니다. 저도 처음엔 API 를 못 쓰니 불가능하다고 생각했습니다. 그런데 1~8편을 따라오셨다면 답이 보입니다. 모델은 파일이고(1편), 파일을 GPU 에 올려 서버로 띄우면 되고(5편), 문서는 벡터 DB 에서 찾아 붙이면 됩니다(8편). 전부 회사 안에서 할 수 있는 일입니다. 그럼 무엇을 안으로 들여와야 하는지부터 정리하겠습니다.
