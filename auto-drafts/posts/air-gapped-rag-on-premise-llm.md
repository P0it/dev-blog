---
title: 인터넷 선을 뽑은 회사에서 RAG 를 돌리려면 안으로 들여와야 하는 다섯 가지
...
---
> 금융·공공·방산처럼 인터넷이 물리적으로 끊긴 망에서도 생성형 AI 는 됩니다. 다만 8편 RAG 의 부품 — LLM·임베딩 모델·벡터 DB·컨테이너 이미지·패키지 — 을 전부 **밖에서 받아 안으로 옮겨야** 합니다. 반입 절차와 보안 갱신이 이 환경의 핵심 업무입니다.

## Air Gap · 폐쇄망 · On-premise
...
이 환경에서는 OpenAI·Anthropic API 를 호출할 수 없습니다. 데이터가 밖으로 나가면 안 되는 것도 있지만 애초에 나갈 선이 없습니다. 6편의 Egress 가 0 인 상태입니다. 그래서 8편의 RAG 를 구성하는 모든 부품이 망 안에 있어야 합니다.

## 안에 있어야 하는 다섯 가지

8편 구조를 부품별로 나누면 다섯 개가 나옵니다.

| | 밖에서는 | 안에서는 |
|---|---|---|
| LLM | OpenAI API | 6편의 MaaS — vLLM 위에 오픈 모델 |
...
LLM 과 임베딩 모델은 Hugging Face 에서 받는 오픈 모델이어야 합니다. 1편의 Qwen2.5-7B 처럼 라이선스가 Apache-2.0 인 모델이 반입 심사에서 편합니다. 벡터 DB 는 8편에서 말한 대로 이미 Postgres 를 쓰고 있다면 pgvector 가 반입할 게 가장 적습니다.

## 반입 — 밖에서 받아 안으로

NVIDIA 의 NIM 문서가 이 절차를 두 단계로 적어 두고 있습니다. 인터넷이 되는 머신에서 받아 준비하고, 격리된 머신에서 네트워크 없이 실행합니다.
...
1. **모델 파일** — 외부망 PC 에서 `hf download Qwen/Qwen2.5-7B-Instruct` 로 1편의 safetensors 묶음을 받습니다. 임베딩 모델과 Reranker 도 같이 받습니다.
...
4. **해시 기록** — 파일마다 SHA-256 을 적어 둡니다. 반입 심사에서 대조하고, 12편의 SBOM 에 들어갑니다.
...
부품이 다 들어오면 구조 자체는 6편·8편과 같습니다. 다른 점은 모든 화살표가 망 안에서 끝난다는 것뿐입니다.
...
사용자 질문은 내부 앱 → 내부 MaaS API → 임베딩 → 벡터 DB → LLM 을 거쳐 돌아옵니다. 어느 단계에서도 망 밖으로 나가는 호출이 없습니다. 인프라 팀이 이 그림을 한 번 세우면 각 부서는 6편의 MaaS 처럼 API 주소만 받아 씁니다.

## 데이터 유출 방지

인터넷이 끊겼다고 유출 걱정이 끝나는 건 아닙니다. 망 안에서도 **부서 간 경계**가 있습니다. 인사팀 문서를 색인한 벡터 DB 를 영업팀이 검색하면 안 됩니다. 8편의 벡터 DB 에 문서마다 접근 권한을 태그로 붙이고 검색할 때 사용자 권한으로 거르는 것이 기본입니다. 프롬프트와 답변 로그도 남기되 어디에 얼마나 보관할지를 정해 둬야 합니다.

모델 자체도 점검 대상입니다. 1편의 safetensors 형식이 널리 쓰이는 이유 중 하나가 이것입니다. 예전 PyTorch 의 `.bin`(pickle) 형식은 파일을 여는 순간 임의 코드가 실행될 수 있어 반입 심사에서 꺼립니다. safetensors 는 숫자 표만 담고 코드를 담지 않습니다.

## 보안 업데이트가 가장 어려운 일

폐쇄망 운영에서 제일 힘든 건 첫 구축이 아니라 **갱신**입니다. vLLM 이 취약점을 고쳐 새 버전을 내면 그 이미지를 다시 받아 심사하고 반입해야 합니다. 밖에서는 `docker pull` 한 줄인 일이 안에서는 며칠짜리 절차입니다. 모델도 마찬가지입니다. 새 모델이 나올 때마다 15GB 를 옮깁니다.

그래서 "지금 안에 뭐가 들어와 있는가"를 목록으로 갖고 있어야 합니다. 이미지 안의 패키지 버전, 모델 파일의 해시, 그 패키지들의 알려진 취약점. 이 목록이 12편에서 다룰 **SBOM** 입니다. 반입 단계에서 적어 둔 해시가 그 출발점입니다.

여기까지가 LLM 을 회사 안에서 돌리는 인프라의 큰 그림입니다. 다음 편부터는 방향을 바꿔 그 LLM 에 넣을 **데이터**가 어디에 어떻게 쌓이는지, 데이터 플랫폼 쪽을 봅니다.
