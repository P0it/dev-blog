...
---
> 금융·공공·방산처럼 인터넷이 물리적으로 끊긴 망에서도 생성형 AI 는 됩니다. 다만 8편 RAG 의 부품 — LLM, 임베딩 모델, 벡터 DB, 컨테이너 이미지, 패키지 — 다섯 가지를 전부 밖에서 받아 안으로 옮겨야 합니다. 이 반입 절차와 그 뒤의 보안 갱신이 폐쇄망 AI 운영의 핵심 업무입니다.

금융·공공·방산 쪽에서 일하시는 분들은 "우리는 인터넷이 안 되는데 ChatGPT 같은 걸 쓸 수 있나요?"라는 질문을 자주 받으실 겁니다. 저도 처음엔 API 를 못 쓰니 불가능하다고 생각했습니다. 그런데 1~8편을 따라오셨다면 답이 보입니다. 됩니다. 모델은 파일이고(1편), 파일을 GPU 에 올려 서버로 띄우면 되고(5편), 문서는 벡터 DB 에서 찾아 붙이면 됩니다(8편). 전부 회사 안에서 할 수 있는 일입니다. 문제는 그 부품들을 어떻게 안으로 들여오느냐입니다.
...
그럼 이 환경에서 뭐가 안 될까요? OpenAI·Anthropic API 를 호출할 수 없습니다. 데이터가 밖으로 나가면 안 되는 것도 있지만 애초에 나갈 선이 없습니다. 6편의 Egress 가 0 인 상태입니다. 그래서 8편의 RAG 를 구성하는 모든 부품이 망 안에 있어야 합니다.

## 안에 있어야 하는 다섯 가지

8편 구조를 부품별로 나누면 다섯 가지가 나옵니다. 밖에서 쓰던 것과 안에서 대신 쓸 것을 나란히 놓으면 이렇습니다.
...
아닙니다. 망 안에서도 **부서 간 경계**가 있습니다. 인사팀 문서를 색인한 벡터 DB 를 영업팀이 검색하면 안 됩니다. 8편의 벡터 DB 에 문서마다 접근 권한을 태그로 붙이고 검색할 때 사용자 권한으로 거르는 것이 기본입니다. 프롬프트와 답변 로그도 남기되 어디에 얼마나 보관할지를 정해 둬야 합니다.
...
여기까지가 LLM 을 회사 안에서 돌리는 인프라의 큰 그림입니다. 그럼 그 LLM 에 넣을 데이터는 어디에 어떻게 쌓여 있을까요? 다음 편부터는 방향을 바꿔 데이터 플랫폼에 대해서 알아보겠습니다.
