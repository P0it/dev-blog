...
---
> 문서를 잘게 잘라 숫자 벡터로 바꿔 벡터 DB 에 넣어 두고 질문이 오면 비슷한 조각을 찾아서 프롬프트에 붙여 주는 방식입니다. 모델 파라미터는 한 개도 안 바뀝니다. 이 흐름을 알면 RAG 의 품질이 LLM 이 아니라 검색에서 갈린다는 것, 그리고 임베딩 모델·벡터 DB·Reranker 가 각각 어디에 끼는지 보입니다.

LLM 은 학습이 끝난 뒤의 일도, 회사 안의 문서도 모릅니다. 사내 규정을 물으면 모른다고 하거나 그럴듯하게 지어냅니다. 7편에서 파인튜닝은 이 문제의 답이 아니라고 했는데 그럼 뭐가 답일까요? 저도 처음엔 "문서를 통째로 프롬프트에 넣으면 되지 않나" 싶었습니다. 그게 3편의 컨텍스트 한계에 걸린다는 걸 알고 나서야 RAG 가 왜 이렇게 생겼는지 이해가 됐습니다. 그 구조를 색인부터 답변까지 따라가 보겠습니다.
