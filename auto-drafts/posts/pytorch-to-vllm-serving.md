...
1편에서 파일 목록을 봤던 그 저장소가 배포 채널입니다. Hugging Face Hub 는 모델 파일을 git 저장소처럼 관리하는 곳이고 `transformers` 라이브러리가 이름만 주면 파일을 받아 메모리에 올려 줍니다. 그 아래에서 실제 행렬 계산을 하는 프레임워크가 **PyTorch** 입니다. 학계·업계 대부분의 LLM 이 PyTorch 로 만들어지고 배포됩니다.
...
여기까지가 GPU 서버 한 대 안의 이야기입니다. 그럼 GPU 가 여러 장, 서버가 여러 대가 되면 어떻게 될까요? 다음 편에서 vLLM 위에 얹는 층인 Kubernetes·MIG·llm-d 에 대해서 알아보겠습니다.
