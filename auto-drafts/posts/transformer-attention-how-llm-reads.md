...
여기까지 오면 파일 뷰어에서 이상했던 숫자 하나가 풀립니다. `q_proj.weight` 는 `[3584, 3584]` 인데 `k_proj.weight` 는 `[512, 3584]` 로 작았습니다. Qwen 의 `config.json` 에 `num_key_value_heads: 4` 가 있는데 Query 헤드는 28개를 두고 Key·Value 헤드는 4개만 둬서 여러 Query 가 나눠 쓰는 방식(GQA)입니다. 그래서 K·V 표가 Q 표의 1/7 크기입니다. 이 설계가 추론 때 GPU 메모리에 쌓이는 KV Cache 크기를 결정합니다.

## 결국 확률 하나를 내는 다음 토큰 예측
