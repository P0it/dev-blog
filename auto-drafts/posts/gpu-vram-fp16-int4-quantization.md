---
title: 7B 모델은 왜 GPU 메모리가 14GB나 필요할까요?
...
---
> 파라미터 76억 개 × 숫자 하나당 2바이트(FP16) = 약 15GB 이기 때문입니다. 그래서 숫자 하나를 1바이트(INT8)나 0.5바이트(INT4)로 줄이면 같은 모델이 7.6GB, 3.8GB 가 됩니다. 이 줄이는 작업이 양자화이고 어느 GPU 에서 돌아가는지와 답변 품질을 함께 결정합니다.

Hugging Face 에서 같은 Qwen2.5-7B 인데 어떤 저장소는 15.2GB, 어떤 저장소는 4.68GB 짜리 파일을 올려 둔 걸 보셨을 겁니다. 파라미터 수는 똑같이 76억 개인데 왜 용량이 세 배 차이가 날까요? 저도 처음엔 작은 쪽이 뭔가 잘라낸 모델인 줄 알았습니다. 그게 아니라 **숫자 하나를 몇 바이트로 적느냐**가 다른 겁니다. 그 전에 GPU 메모리가 왜 중요한지부터 잡고 가겠습니다.
...
## 모델이 통째로 들어가야 하는 메모리 — VRAM

GPU 에는 CPU 의 RAM 과 별개로 자기 메모리가 붙어 있습니다. 이것이 **VRAM** 입니다. 소비자용 RTX 4090 은 24GB, 데이터센터용 H100 은 80GB 입니다. 그런데 왜 모델이 여기에 "통째로" 들어가야 할까요? 3편에서 본 대로 Decode 는 매 토큰마다 가중치 전부를 읽습니다. CPU RAM 에 두고 그때그때 옮기면 수십 배 느려집니다. 그래서 **가중치가 VRAM 에 전부** 올라가 있어야 합니다.

VRAM 을 쓰는 것은 가중치만이 아닙니다. 3편의 KV Cache 도 여기 올라가고 계산 중간값(활성화)도 공간을 차지합니다. 그래서 "7B 는 14GB 니까 16GB GPU 면 된다"는 계산은 보통 빗나갑니다. 가중치를 올리고 남는 공간이 KV Cache 몫이고 그 공간이 동시에 처리할 수 있는 요청 수를 정합니다.

## 숫자 하나가 몇 바이트인지 — 부동소수점
...
여기까지가 모델 파일 자체의 이야기입니다. 그럼 이 파일을 실제로 Python 에서 불러 GPU 에 올리면 어떻게 될까요? 다음 편에서 그걸 직접 해 보고 왜 vLLM 같은 추론 서버가 따로 필요한지 알아보겠습니다.
