...
---
> 같은 Qwen2.5-7B 인데 어떤 저장소는 15.2GB, 어떤 저장소는 4.68GB 입니다. 파라미터 수는 똑같이 76억 개인데 숫자 하나를 몇 바이트로 적느냐가 다릅니다. 이 차이를 만드는 게 양자화이고 어느 GPU 에서 돌아가는지와 답변 품질을 함께 결정합니다.
...
여기까지가 모델 파일 자체의 이야기입니다. 다음 편에서는 이 파일을 실제로 Python 에서 불러 GPU 에 올려 보고 왜 vLLM 같은 추론 서버가 따로 필요한지 알아보겠습니다.
