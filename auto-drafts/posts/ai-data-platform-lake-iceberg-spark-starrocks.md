...
---
> 대부분 DB 가 아니라 오브젝트 스토리지(S3)에 Parquet 파일로 쌓입니다. 그 파일 더미를 테이블처럼 다루게 해 주는 것이 Iceberg 이고 그 위에서 큰 처리는 Spark 가, 빠른 질의는 StarRocks 가 맡습니다. 이 네 가지의 역할만 잡으면 "데이터 플랫폼"이라는 말이 한 장의 그림으로 이어집니다.

8편 RAG 에 넣을 문서, 7편 파인튜닝에 쓸 대화 로그, 다음 편 예측 모델의 학습 데이터. 이런 것들은 회사 어디에 있을까요? 저는 당연히 데이터베이스에 있을 거라고 생각했는데 실제로 AI 팀이 데이터를 가져오는 곳은 DB 가 아니라 S3 같은 파일 저장소였습니다. 왜 그런지, 그리고 파일로 쌓아 두면 뭐가 문제라서 Iceberg 같은 게 필요한지 순서대로 따라가 보겠습니다.
