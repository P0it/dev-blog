...
---
> 첫 글자가 늦는 건 프롬프트 전체를 한 번에 계산하는 Prefill 단계 때문이고 그 뒤가 빠른 건 Decode 단계가 앞에서 계산한 Key·Value 를 KV Cache 에 저장해 두고 새 토큰 것만 계산하기 때문입니다. 이 두 단계를 알면 TTFT·Tokens/sec 같은 지표가 뭘 재는지, 긴 문맥이 왜 GPU 메모리를 잡아먹는지 보입니다.

챗봇에 긴 문서를 붙여 넣고 질문해 보면, 첫 글자가 나오기까지 한참 걸리다가 일단 나오기 시작하면 줄줄 흘러나옵니다. 저는 이게 그냥 네트워크 지연인 줄 알았는데 아니었습니다. 모델이 답을 만드는 과정이 **두 단계로 나뉘어 있고 두 단계의 병목이 다르기** 때문입니다. 2편에서 본 "다음 토큰 하나 고르기"에서 출발해 그 두 단계를 따라가 보겠습니다.
