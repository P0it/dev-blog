---
title: 에이전트 256대를 띄우기 전에 Bash diff 부터 봐야 하는 이유
slug: claude-code-bash-diff-self-report
tags: [AI 코딩, AI 에이전트, Claude Code]
category: insights
---

> Claude Code 2.1.269 릴리스 노트에 두 항목이 나란히 들어왔습니다. 동시 에이전트 상한을 1~256 범위에서 올리는 설정과, Bash 명령이 바꾼 파일의 diff 를 도구 결과에 붙이는 변경입니다. 지금까지 명령이 파일에 무엇을 했는지는 아무도 다시 확인하지 않았습니다. 남은 건 모델이 "고쳤습니다"라고 적은 문장뿐이었습니다. diff 는 파일이 실제로 어떻게 바뀌었는지를 모델에게 보여 줍니다. 상한을 올리는 일은 이 뒤에 와야 합니다.

## 2.1.269 에 나란히 들어온 두 항목

9월 11일 [Claude Code 2.1.269](https://code.claude.com/docs/en/changelog) 에 항목이 100개 넘게 올라왔습니다. 그중 두 개가 서로 반대 방향입니다.

- `CLAUDE_CODE_WORKFLOW_MAX_CONCURRENT_AGENTS`: Workflow 도구의 동시 에이전트 한도를 **1~256** 범위에서 정한다
- `bashEditDiffEnabled`: Bash 도구가 파일을 고쳤을 때 **바뀐 파일의 diff** 를 도구 결과에 붙인다

같은 릴리스에 `claude plugin eval` 과 `/output-style [name]` 도 함께 들어왔습니다. 그런데 위의 둘은 방향이 정반대입니다. 하나는 더 많이 실행하는 설정이고 하나는 실행한 결과를 다시 확인하는 설정입니다.
...
도구 루프의 한 턴은 구조가 단순합니다. 모델이 Bash 명령을 호출합니다. 하네스가 stdout 과 stderr, 종료 코드를 돌려주면 모델은 그걸 읽은 다음 "수정했습니다"라고 적습니다.

이 문장을 적는 시점에 모델은 파일 시스템을 본 적이 없습니다. 명령이 실제로 무엇을 바꿨는지 아무도 다시 확인하지 않았거든요. 하네스가 받은 것은 명령의 출력이고 사람이 읽는 것은 모델이 그 출력을 요약한 말입니다.

실사용 세션 20,574건을 분석한 [연구](https://arxiv.org/abs/2605.29442)도 같은 문제를 지적합니다. 코딩 에이전트의 실패 유형을 분류하면서 **부정확한 자기 보고**를 따로 한 항목으로 뒀습니다. 실행하지 않은 동작을 했다고 적는 경우, 부분적으로 끝난 상태를 완료로 보고하는 경우, 다룬 범위를 실제보다 넓게 말하는 경우가 여기 들어갑니다.

```illustration
<svg viewBox="0 0 800 450" xmlns="http://www.w3.org/2000/svg">
  <title>모델과 하네스 사이에서만 도는 루프와 그 밖에 있는 파일 시스템</title>
...
## stdout 은 프로그램의 출력이고 diff 는 파일의 변경입니다

둘은 같은 사건을 가리키지만 나오는 곳이 다릅니다.

stdout 은 실행된 프로그램이 스스로 출력한 문장입니다. 아무 파일도 건드리지 않는 스크립트가 `Done.` 을 출력하게 만드는 건 어렵지 않습니다. 종료 코드 0 도 마찬가지입니다. 무언가 일어났다는 증거가 아니라 무언가 일어났다는 프로그램의 주장입니다.

더 흔한 경우는 아예 표시가 나지 않습니다. `sed -i` 가 찾으려던 패턴과 한 글자라도 어긋나면 바뀌는 줄은 하나도 없는데, 출력은 비어 있고 종료 코드는 0 입니다. 겉으로는 성공과 똑같습니다. 이 결과만 받은 모델이 다음 턴에 "치환했습니다"라고 적는 건 거짓말이라기보다 확인 수단이 없어서입니다.

diff 는 명령이 끝난 뒤 파일의 전후 상태를 비교해서 만듭니다. 프로그램이 무엇을 출력했든 상관없습니다. 이걸 도구 결과에 붙이면 모델은 자기가 실행한 명령의 결과를 두 가지 정보로 받습니다. 프로그램의 출력과 파일의 실제 변경입니다. 둘이 어긋나면 그 턴에서 바로 보입니다.

```illustration
<svg viewBox="0 0 800 450" xmlns="http://www.w3.org/2000/svg">
  <title>같은 명령에서 나오는 두 정보: 프로그램의 출력과 파일의 실제 변경</title>
...
에이전트 하네스에서 검증은 대개 맨 뒤에 몰려 있습니다. 테스트와 린트를 실행하고 사람이 PR 을 여는데, 전부 여러 턴이 지난 뒤입니다. diff 를 붙이면 그 검증의 일부가 **명령 하나 단위로** 앞당겨집니다. 잘못된 변경을 스무 턴 뒤가 아니라 그 턴에서 확인합니다.

## 256 까지 열어 둔 상한과 검토 부채

동시 에이전트 한도를 1에서 256까지 열어 둔 건 팬아웃을 정식 설정 항목으로 인정했다는 뜻입니다. 그런데 팬아웃을 늘리면 늘어나는 쪽은 결과물이지 검토 용량이 아닙니다.

병렬 에이전트를 여러 개 운영해 본 쪽의 집계는 대체로 비슷합니다. 에이전트 수를 올릴수록 조정 비용이 먼저 붙어 수확이 체감됩니다. [병목이 코드 리뷰로 옮겨 갔다](https://moderne.ai/blog/ai-didnt-break-coding-it-broke-code-review)는 관찰도 여러 곳에서 나옵니다. AI 가 작성한 PR 이 더 크고 더 오래 대기한다는 [집계](https://www.flowverify.co/blog/ai-code-review-bottleneck-2026-data)도 있습니다. 수치는 출처마다 다르니 경향으로만 읽는 게 맞습니다.
...
에이전트 한 개가 자기 보고를 적으면 사람이 읽고 넘어갑니다. 256개가 자기 보고를 적으면 아무도 읽지 않습니다. 그래서 256 은 처리량의 한도라기보다 검토 부채의 한도입니다.

## diff 의 비용과 2.1.270 의 회귀

diff 를 붙이는 일이 공짜는 아닙니다. 비용은 컨텍스트로 치릅니다.

포맷터를 한 번 실행하거나 의존성을 설치하면 바뀐 파일이 수백 개가 되고 그 diff 는 그대로 토큰이 됩니다. 정작 모델이 봐야 할 변경이 무관한 변경에 밀려나면, 확인 수단을 붙이려다 확인 수단을 잃습니다. 어떤 명령의 diff 를 붙이고 어디서 자를지가 곧 설계 문제가 됩니다.

```illustration
<svg viewBox="0 0 800 450" xmlns="http://www.w3.org/2000/svg">
  <title>대량 diff 가 컨텍스트 창을 채워 정작 봐야 할 변경을 밀어내는 모습</title>
...
문제도 바로 나타났습니다. 하루 뒤인 9월 12일 [2.1.270](https://code.claude.com/docs/en/changelog) 이 고친 항목은 하나입니다. 세션이 한동안 돌아간 뒤 읽기 전용 git 명령이 갑자기 권한을 묻던 문제, 2.1.269 의 회귀입니다. 릴리스 노트가 원인을 어느 변경으로 지목하지는 않았습니다. 다만 Bash 도구 주변을 손본 릴리스에서 다음 날 회귀가 나왔다는 사실은 남습니다.

더 센 반론은 따로 있습니다. diff 를 모델에게 보여 주는 방식 자체가 약하다는 지적입니다. 자기 보고가 어긋나는 건 정보가 없어서만이 아니라 있는 정보를 건너뛰기 때문이기도 합니다. 그렇다면 같은 모델에게 근거를 하나 더 주는 것으로는 판정자가 바뀌지 않습니다. 판정을 모델 밖으로 빼서 테스트나 CI 처럼 사람도 에이전트도 손댈 수 없는 곳에 두는 편이 낫다는 주장입니다.

맞는 말입니다. 다만 그 판정은 대개 작업이 한 덩어리로 끝난 뒤에야 돌아옵니다. 그사이 에이전트는 틀어진 전제 위에서 열 턴을 더 진행하고 CI 가 실패했을 때는 어디서부터 어긋났는지 되짚어야 합니다. diff 는 그 판정을 대신하지 않습니다. 판정이 나오기 전까지의 구간을 확인할 수 있게 해 줄 뿐입니다. 둘은 서로 경쟁하지 않습니다.

## diff 가 먼저이고 256 이 나중입니다

이 관점에서 보면 하네스를 직접 운영하는 쪽이 먼저 손볼 곳은 분명합니다. 에이전트를 몇 개까지 띄울지는 그다음입니다.

에이전트 한 개가 무엇을 했는지 모델의 말과 별개로 확인할 수 있어야, 열 개를 띄운 결과도 읽을 수 있습니다. 그 수단이 없으면 팬아웃은 검토할 수 없는 산출물을 빠르게 쌓는 장치가 됩니다. 상한을 256까지 열어 둔 릴리스에 diff 가 같이 실린 순서를, 저는 그렇게 읽었습니다.
