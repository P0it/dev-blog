...
그럼 "모델"은 뭘까요? 머신러닝에서 모델은 입력을 받아 출력을 내는 함수입니다. 함수의 모양은 코드가 정하고 함수 안의 값은 **파라미터**가 정합니다. 파라미터는 학습으로 정해진 숫자 하나하나를 말합니다. 그중 입력에 곱해지는 숫자를 **가중치**(Weight), 더해지는 숫자를 **바이어스**(Bias)라고 나눠 부르는데 실무에서는 둘을 합쳐 그냥 가중치라고 부릅니다.

이름 뒤의 **7B·14B·70B** 는 이 파라미터가 몇 십억 개인지를 뜻합니다. Hugging Face 가 Qwen2.5-7B 파일을 읽어 센 실제 개수는 **7,615,616,512 개**입니다. 7B 는 반올림한 이름이고 진짜는 76억 개입니다.
