...
-- 기본값 없이 먼저 붙인다. default 'draft' 를 달고 만들면 이미 공개돼 있던
-- 프로젝트까지 draft 로 떨어져 /lab 에서 사라진다.
alter table projects
  add column if not exists visibility text;

-- 지금 올라가 있는 프로젝트는 전부 공개 상태였다.
update projects set visibility = 'published' where visibility is null;

-- 앞으로 들어오는 행만 draft 로 시작한다.
alter table projects alter column visibility set default 'draft';
