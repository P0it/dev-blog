---
name: portfolio
description: 이 레포의 프로젝트를 포트폴리오 원고(portfolio.md)로 정리한다. 레포 구조·커밋 로그·세션 메모리에서 기획(문제·타겟·해법·범위·접은 안), 만든 사람 인터뷰, 유저 플로우, 개발 과정, 붙인 API, 시행착오를 뽑아 정해진 템플릿으로 채운다. 사용자가 "포트폴리오 정리", "이 프로젝트 포트폴리오로 뽑아줘", "/portfolio" 라고 하면 쓴다.
---
...
3. **사용자에게 묻는다** — 기획은 코드에 안 남는다. 무엇이 문제였고, 누구를 생각했고,
   ...
   특히 `문제`·`타겟`·`접은 안` 은 커밋에서 거의 안 읽히니 반드시 묻는다. 지어내지 않는다.
...
**접은 안** 폰에서 바로 모델을 부르는 안은 접었어요. 키를 폰에 두기 싫었습니다
...
그다음에 어디까지만 하고(`범위`) 무엇을 먼저 놓고(`우선순위`) 무엇을 접었는지(`접은 안`)로
간다. **앞의 셋이 빠지면 이 섹션은 "무엇을 뺐는지 해명하는 글"이 된다** — 실제로 그렇게
...
판단이 해명처럼 읽힌다. 같은 라벨을 여러 번 써도 된다. 접은 안이 셋이면 세 줄이다.

### 라벨

**적은 순서대로 표에 선다.** 아래 차례를 지킨다 — 세우는 것(문제·타겟·해법)이 먼저,
자르는 것(범위·우선순위)이 다음, 떠받친 조건(제약·정책)이 받고, 버린 길(접은 안)이 닫는다.
...
| `접은 안` | 저울질하다 접은 다른 방법과, 접은 이유 |

표에 없는 라벨을 새로 지어도 화면은 그대로 그린다. 라벨이 그 자리에 그냥 선다.
다만 위 여덟이면 대개 덮이니, 굳이 늘리지 않는다.
...
**`넣지 않은 것` 은 쓰지 않는다.** `범위` 가 이미 경계를 긋는데 그 바깥을 또 한 줄로
적으면 같은 말이 두 번 선다. 안 하기로 한 게 판단이면 `범위` 한 줄에 같이 적고,
저울질한 끝에 버린 것이면 `접은 안` 이다.

**넷은 되도록 넣는다** — `문제` `타겟` `해법` `범위`. 앞의 셋이 빠지면 해명하는 글이
되고, `범위` 가 빠지면 무한정 만든 것처럼 읽힌다. `우선순위`·`접은 안` 은 있으면 판단이
가장 잘 드러나는 자리라 물어서라도 채운다. **넷에서 여덟 줄이면 넉넉하다.**
...
- **기획 시점에서 얼린다.** `## 구상` 과 같은 규칙이다. 첫 커밋 즈음에 내린 판단만 남긴다.
  ...
  `## 개발 과정` 의 재료다
- **`접은 안` 은 지어내기 제일 쉽다.** 커밋·이슈·세션 기록·사용자 답에 저울질한 흔적이
  없으면 그 줄을 뺀다. 있지도 않았던 후보를 세워 두고 접었다고 쓰지 않는다
...
**`문제`·`타겟`·`해법`·`범위`·`우선순위`·`제약`·`정책`·`접은 안` 은 여기 쓰지 않는다.**
...
- **`접은 안`·`범위` 는 특히 없다.** 안 고른 길은 코드에 안 남는다. 물어서 나온 것만 쓴다
...
- 기획에 넣은 라벨과, `범위`·`우선순위`·`접은 안` 중 근거가 없어 뺀 것
