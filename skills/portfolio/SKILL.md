---
name: portfolio
description: 이 레포의 프로젝트를 포트폴리오 원고(portfolio.md)로 정리한다. 레포 구조·커밋 로그·세션 메모리에서 기획 판단(범위·우선순위·접은 안), 만든 사람 인터뷰, 유저 플로우, 개발 과정, 붙인 API, 시행착오를 뽑아 정해진 템플릿으로 채운다. 사용자가 "포트폴리오 정리", "이 프로젝트 포트폴리오로 뽑아줘", "/portfolio" 라고 하면 쓴다.
---
...
- **기획** — 어디까지만 풀기로 했고, 무엇을 먼저 놓았고, 무엇을 접었는지
- **개발 과정** — 생각이 흘러간 순서. 막힌 지점과 그때 붙인 것
- **시행착오** — 그중에서도 유난히 헤맸던 개별 사건의 부검

`## 인터뷰` 는 이 셋을 사람 목소리로 받는 자리다. 왜 시작했고 누가 쓰는지.
**판단은 기획, 이야기는 인터뷰** — 이 둘을 한 섹션에 섞지 않는다.
...
3. **사용자에게 묻는다** — 기획 의도는 코드에 안 남는다. 어디까지만 하기로 했는지,
   무엇을 저울질하다 접었는지, 왜 만들었는지, 누구를 생각했는지는 레포에서 못 캐면
   **그냥 물어본다.** `## 기획` 과 `## 인터뷰` 의 라벨 표(아래)를 질문지로 쓴다.
   특히 `범위`·`접은 안` 은 커밋에서 거의 안 읽히니 반드시 묻는다. 지어내지 않는다.
...
`## 기획` 바로 앞에 온다. 무엇을 만들기로 했는지를 먼저 보여주고, 그걸 어디까지만
하기로 했는지가 뒤에서 받는 자리라, 둘을 떼어 놓으면 구상만 붕 뜬다.
**구상은 만들 것의 목록이고, 기획은 그 목록을 그렇게 자른 이유다.**

**여기는 아직 전문적인 자리가 아니다.** 막연히 떠올린 프로토타입의 기능 목록이고,
판단·근거·경계가 들어가는 자리는 바로 뒤 `## 기획` 이다. 층을 섞지 않는다.
...
**여기부터 전문적인 자리다.** 앞의 `## 구상` 이 막연한 첫 그림이라면, 여기는 그 그림을
실제로 자른 결정과 근거다. **여기가 판단이다.** 만들기 전에 무엇을 어디까지 하기로 했고, 무엇을 먼저 놓았고,
검토했다 접은 안은 무엇이었는지를 적는다. **왜 시작했는지·누가 쓰는지 같은 사람 이야기는
여기가 아니라 `## 인터뷰` 다.** 예전에는 이 제목이 그 문답을 달고 있었는데, 이름만 기획이고
내용은 소개·계기·사용자라 정작 기획이 빠져 있었다. 그래서 둘을 뗐다.
...
**적은 순서대로 표에 선다.** 아래 차례를 지킨다 — 무엇을 하기로 했는지(범위·우선순위)가
먼저고, 그 판단을 떠받친 것(제약·정책)이 받고, 버린 길(접은 안)이 닫는다.
...
**셋은 되도록 넣는다** — `범위` `우선순위` `접은 안`. 이 셋이 없으면 이 섹션은
다시 소개가 된다. 나머지는 근거가 있을 때만. **셋에서 여섯 줄이면 넉넉하다.**
...
- **`## 구상` 의 항목을 라벨만 갈아 옮기지 않는다.** 구상은 막연히 떠올린 기능 목록이고,
  여기는 그 목록을 그렇게 자른 결정과 근거다. 층이 다르다
...
- 막힌 지점과 해결 → `## 시행착오`
- 왜 시작했는지·누가 쓰는지·만들고 나서 어땠는지 → `## 인터뷰`

**옛 원고를 손보는 중이라면** — 예전 `## 기획` 은 통째로 문답이다. 제목을 `## 인터뷰` 로
바꾸고, 그 안에 섞여 있던 `제약`·`정책` 을 새 `## 기획` 으로
올린다. 새 `## 기획` 은 인터뷰 **앞**에 선다. 판단이 비면 지어내지 말고 사용자에게 묻는다.
...
**`범위`·`우선순위`·`제약`·`정책`·`접은 안` 은
여기 쓰지 않는다.** 판단은 `## 기획` 자리다.
...
- **사용자** — 누가 언제 쓰는지. 혼자 쓰는 도구면 그렇게 쓴다.
  있지도 않은 페르소나를 만들지 않는다
