...
**넣지 않은 것** 예약 발행은 안 붙였어요. 쓰고 고치는 데까지만 봅니다
...
**여기가 판단이다.** 만들기 전에 무엇을 어디까지 하기로 했고, 무엇을 먼저 놓았고,
...
| 라벨 | 여기 적는 것 |
|---|---|
| `범위` | 어디까지만 풀기로 했는지. 경계를 문장으로 |
| `우선순위` | 무엇을 먼저 만들기로 했고, 왜 그게 먼저였는지 |
| `접은 안` | 저울질하다 접은 다른 방법과, 접은 이유 |
| `정책` | 애매한 경우를 어떻게 처리하기로 정했는지 (중복·실패·빈 값·한도 초과) |
| `성공 기준` | 어디까지 되면 됐다고 보기로 했는지 |
| `가설` | 될 거라고 본 근거 |
| `제약` | 시간·비용·API 한도처럼 판단을 좁힌 조건 |
| `넣지 않은 것` | 할 수 있었는데 지금은 안 하기로 한 것 |
...
- `범위` 와 `넣지 않은 것` 이 같은 말이 되면 한 줄만 남긴다. 앞은 경계, 뒤는 그 바깥에서
  특별히 짚어 둘 것이다
...
바꾸고, 그 안에 섞여 있던 `성공 기준`·`가설`·`제약`·`넣지 않은 것` 을 새 `## 기획` 으로
...
**`범위`·`우선순위`·`접은 안`·`정책`·`성공 기준`·`가설`·`제약`·`넣지 않은 것` 은
