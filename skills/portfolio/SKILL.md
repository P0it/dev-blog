...
**성공 기준** 폰에서 던진 글이 손 안 대고 발행까지 가면 됐다고 봤어요
...
먼저고, 그 판단을 떠받친 것(가설·제약·정책)이 받고, 무엇으로 됐다고 볼지(성공 기준)와
버린 길(접은 안)이 닫는다.
...
바꾸고, 그 안에 섞여 있던 `성공 기준`·`가설`·`제약` 을 새 `## 기획` 으로
