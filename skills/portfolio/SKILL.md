...
3. **사용자에게 묻는다** — 기획 의도는 코드에 안 남는다. 왜 만들었는지, 누구를 생각했는지,
   얼마나 걸렸는지는 레포에서 못 캐면 **그냥 물어본다.**
   `## 기획` 의 라벨 표(아래)에 있는 질문을 그대로 던지면 된다. 지어내지 않는다.
...
`데이터와 API` → `구상` → `기획` → `유저 플로우` → `구조` →
`개발 과정` → `시행착오` → `남은 것`

- **원고는 `## 화면` 으로 연다.** 이게 어떻게 생긴 물건인지가 먼저다.
  설명 문단으로 열지 않는다 — 소개는 `## 기획` 의 첫 문답이 맡는다
...
**넣지 않은 것** 통계도 예약 발행도 없어요. 쓰고 고치는 데만 붙였습니다
...
이 물건이 뭔지를 표지에서 한 번, `## 기획` 에서 또 한 번 말하게 돼서 같은 이야기가
두 번 나왔기 때문이다. 지금은 `## 기획` 의 첫 줄 `**소개**` 가 그 자리를 대신한다 —
"어떤 서비스인가요?"라는 질문에 답하는 꼴로 그려진다.

- 원고에 `## 제품 소개` 를 쓰지 않는다. 써 두면 화면에 아예 안 그려지고 흘러간다
- 표지에 뜨는 한 줄은 프런트매터 `tagline` 이다. 소개 문단이 아니다
- 옛 원고를 손보는 중이라면 그 문단을 줄여 `## 기획` 의 `**소개**` 로 옮긴다
...
**이 섹션이 인터뷰다.** 라벨을 쓰면 화면에서 질문으로 바뀌고, 값이 그 답으로 붙는다.
...
**나머지는 근거가 있을 때만 넣는다.** 여섯에서 여덟 줄이면 넉넉하다. 열 줄을 넘기면
