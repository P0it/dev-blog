...
3. **사용자에게 묻는다** — 기획 의도는 코드에 안 남는다. 왜 만들었는지, 누구를 생각했는지,
   얼마나 걸렸는지, 제일 마음에 드는 부분이 어딘지는 레포에서 못 캐면 **그냥 물어본다.**
...
**기간** 주말 이틀로 굴러가는 것까지 만들고, 그 뒤로 틈틈이 손봤어요
**자랑** 큐에 걸어 두고 폰을 닫아도 워커가 알아서 끝내 놓는 부분이요
...
답이 개조식이면 대화가 안 된다. `**기간** 2 주`(X) → `**기간** 주말 이틀로 굴러가는
것까지 만들고, 그 뒤로 틈틈이 손봤어요`(O).
...
인터뷰가 아니라 설문지가 된다. 특히 `자랑`·`반응`·`배운 것`·`지금` 은 읽는 사람이
