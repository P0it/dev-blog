...
인터뷰가 아니라 설문지가 된다. 특히 `반응`·`배운 것`·`지금` 은 읽는 사람이
제일 궁금해하는 자리라 한 줄이라도 있으면 글이 사람 목소리로 읽힌다.

**만드는 데 걸린 기간과 제일 마음에 드는 부분은 묻지 않는다.** 라벨 표에서 뺐다.
기간은 혼자 틈틈이 만든 물건에서 뜻이 없는 숫자고, 자랑은 어느 프로젝트에나 같은 톤의
