---
name: portfolio
description: 이 레포의 프로젝트를 포트폴리오 원고(portfolio.md)로 정리한다. 레포 구조·커밋 로그·세션 메모리에서 기획(문제·타겟·해법·범위·우선순위), 만든 사람 인터뷰, 유저 플로우, 개발 과정, 붙인 API, 시행착오를 뽑아 정해진 템플릿으로 채운다. 사용자가 "포트폴리오 정리", "이 프로젝트 포트폴리오로 뽑아줘", "/portfolio" 라고 하면 쓴다.
---
...
**우선순위** 폰에서 던지는 흐름을 먼저 세웠습니다. 결과를 보는 화면은 그다음이었어요
...
그다음에 어디까지만 하고(`범위`) 무엇을 먼저 놓았는지(`우선순위`)로 간다. **앞의 셋이 빠지면 이 섹션은 "무엇을 뺐는지 해명하는 글"이 된다** — 실제로 그렇게
쓰인 원고가 여럿이었다. 뺄셈만 있고 덧셈이 없으면 기획이 아니다.
...
자르는 것(범위·우선순위)이 다음, 떠받친 조건(제약·정책)이 닫는다.

| 라벨 | 여기 적는 것 |
|---|---|
| `문제` | 무엇이 안 풀려 있었는지. 사연이 아니라 상태로 — "…할 방법이 없다", "…만 나온다" |
| `타겟` | 누가 언제 쓰는 물건인지. 혼자 쓰려고 만들었으면 그렇게 쓴다 |
| `해법` | 그래서 어떤 방식으로 풀기로 했는지. 기능 목록이 아니라 **접근** 한 줄 |
| `범위` | 어디까지 풀고 어디부터는 안 풀지. 경계와 그 이유를 한 문장으로 |
| `우선순위` | 무엇을 먼저 세웠고, 왜 그게 먼저였는지 |
...
나중으로 민 것은 `우선순위`(또는 `## 남은 것`)가 맡는다. 그 바깥을 또 한 줄로 적으면
...
**넷은 되도록 넣는다** — `문제` `타겟` `해법` `범위`. 앞의 셋이 빠지면 해명하는 글이
되고, `범위` 가 빠지면 무한정 만든 것처럼 읽힌다. `우선순위` 는 있으면 판단이 가장 잘
드러나는 자리라 물어서라도 채운다. **넷에서 일곱 줄이면 넉넉하다.**
...
**`문제`·`타겟`·`해법`·`범위`·`우선순위`·`제약`·`정책` 은 여기 쓰지 않는다.**
