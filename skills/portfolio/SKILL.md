...
뒤는 **데이터가 흐르는 길**이다. 같은 다이어그램을 두 번 넣지 않는다.
...
사용자가 목적을 이루기까지 밟는 화면과 판단을 mermaid `flowchart LR` 로 그린다.

- **주인공은 사람이다.** 노드에 함수명·테이블명·서비스명을 쓰지 않는다.
  `POST /api/jobs`(X) → `초안 요청`(O)
- 분기(`{…}`)를 최소 하나는 넣는다. 갈림길이 없으면 그건 플로우가 아니라 목록이다
- 노드 8개를 넘기지 않는다. 넘으면 흐름 하나만 남기고 나머지는 뺀다
- `### 단계` 설명은 선택이다. 다이어그램만으로 읽히면 굳이 붙이지 않는다
