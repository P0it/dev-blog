...
끼어드는 자리)** → `데이터와 API` → `구조` → `구상` → `기획` → `유저 플로우` →
`개발 과정` → `시행착오` → `남은 것`

- **본문은 `화면` 으로 연다.** 이게 어떻게 생긴 물건인지가 먼저다
- 그 뒤 `기술 스택` · `데이터와 API` · `구조` 가 한 덩어리다. 무엇으로 만들었고,
  무엇을 끌어다 썼고, 어떻게 얽혀 있는지가 이어진다
...
| `**방식**` | 아래 작은 칩 | `REST · 매일 자동` 처럼 **명사 몇 개**. 문장으로 쓰지 않는다 |
| `**갱신**` | 아래 작은 칩 | `자동` `수동 실행` 수준으로 짧게 |
| `**링크**` | 이름 옆 바깥 링크 아이콘 | 해당 데이터·문서 페이지. 포털 첫 화면 말고 |
