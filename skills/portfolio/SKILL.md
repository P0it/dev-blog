...
블로그가 이 배열을 받아 브랜드 아이콘이 박힌 칩으로 **본문 맨 앞에** 그린다 —
무엇으로 만든 물건인지 먼저 보여주고 이야기로 들어간다.
...
**폰 모양 판을 좌우로 쭉 까는 자리다.** 기획을 읽고 "그래서 이게 어떻게 생겼는데?"
하는 대목에서 훑어보게 한다. `### 화면 이름` + `**파일**` 두 줄이면 끝이다.
...
```
node ~/.claude/skills/portfolio/capture-screens.mjs   --cmd "npm run dev" --base http://localhost:3000   /:홈 /posts:글목록
```

앱을 띄우고, 찍고, 서버를 정리한 뒤 `## 화면` 블록을 찍어 준다. 그 블록을 원고에 붙이고
`**설명**` 줄만 채운다. 결과는 `portfolio/screens/` 로 떨어진다.
