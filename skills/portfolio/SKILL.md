---
name: portfolio
description: 이 레포의 프로젝트를 포트폴리오 원고(portfolio.md)로 정리한다. 레포 구조·커밋 로그·세션 메모리에서 기획(타겟·해법·범위), 만든 사람 인터뷰, 유저 플로우, 개발 과정, 붙인 API, 시행착오를 뽑아 정해진 템플릿으로 채운다. 사용자가 "포트폴리오 정리", "이 프로젝트 포트폴리오로 뽑아줘", "/portfolio" 라고 하면 쓴다.
---
...
- **기획** — 무엇이 문제였고 누구를 위해 어떻게 풀기로 했는지, 그래서 어디까지만
  하고 무엇을 접었는지
- **개발 과정** — 생각이 흘러간 순서. 막힌 지점과 그때 붙인 것
- **시행착오** — 그중에서도 유난히 헤맸던 개별 사건의 부검

`## 인터뷰` 는 이 셋을 사람 목소리로 받는 자리다. 어떤 계기로 시작했고, 만들고 나서
어땠는지. **문제 정의와 타겟은 기획, 사연은 인터뷰** — 같은 이야기라도 앞은 판단으로
쓰고 뒤는 사람 말로 쓴다. 한 섹션에 섞지 않는다.
...
3. **사용자에게 묻는다** — 기획은 코드에 안 남는다. 무엇이 문제였고, 누구를 생각했고,
   ...
   특히 `문제`·`타겟`·`해법` 은 커밋에서 거의 안 읽히니 반드시 묻는다. 지어내지 않는다.
...
**해법** 폰에서 URL 만 던지면 서버가 초안까지 만들어 두는 흐름으로 풉니다
...
**0 에서 1 을 세우는 자리다.** 실무 기획서와 같은 순서로 쓴다 — 무엇이 문제였고(`문제`),
누구를 위한 것이고(`타겟`), 그래서 어떻게 풀기로 했는지(`해법`)를 먼저 세우고,
그다음에 무엇을 넣고 무엇은 안 넣을지(`범위`)로 간다. **덧셈이 먼저다** — `범위` 만
남으면 이 섹션은 "무엇을 뺐는지 해명하는 글"이 된다. 실제로 그렇게 쓰인 원고가 여럿이었다.

**거창하게 쓰지 않는다.** 라벨은 넷이면 충분하고, 한 줄에 한 결정이다. 사업계획서가
아니라 "각 잡고 무엇을 넣을지 정하고 구도를 짠" 자리다.

`문제`·`타겟` 은 사람 사연이 아니라 **정의**로 쓴다. "맥북 앞에 없을 때 글감을 놓쳤어요"는
계기이고 `## 인터뷰` 자리다. 같은 이야기를 여기서는 "글감은 폰에서 떠오르는데 초안을
시작할 자리가 없다"처럼 문제로 세운다.
...
**적은 순서대로 표에 선다.** 아래 차례를 지킨다 — 세우는 것(문제·타겟·해법)이 먼저,
자르는 것(범위)이 다음, 떠받친 조건(제약·정책)이 닫는다.

| 라벨 | 여기 적는 것 |
|---|---|
| `문제` | (선택) 무엇이 안 풀려 있었는지를 상태로. **인터뷰 `계기` 가 이미 말하면 쓰지 않는다** |
| `타겟` | 누가 언제 쓰는 물건인지. 혼자 쓰려고 만들었으면 그렇게 쓴다 |
| `해법` | 그래서 어떤 방식으로 풀기로 했는지. 기능 목록이 아니라 **접근** 한 줄 |
...
다만 위 여섯이면 대개 덮이니, 굳이 늘리지 않는다.

**`해법` 과 `## 구상` 을 헷갈리지 않는다.** 해법은 "인허가 전수로 깔고 3탭 안에 찾게
한다" 같은 **접근**이고, 구상은 그 접근을 기능으로 편 목록이다. 해법 자리에 기능을
나열하면 바로 뒤 구상과 같은 말이 된다.
...
**셋은 되도록 넣는다** — `타겟` `해법` `범위`. 누구를 위해, 어떻게 풀고, 무엇까지
넣는지다. `문제` 는 인터뷰 `계기` 와 겹치면 빼도 된다. **셋에서 여섯 줄이면 넉넉하다.**
...
- **`문제`·`타겟`·`해법`·`범위` 는 특히 없다.** 기획은 코드에 안 남는다. 물어서 나온 것만 쓴다
...
- 기획에 넣은 라벨과, `문제`·`타겟`·`해법`·`범위` 중 근거가 없어 뺀 것
