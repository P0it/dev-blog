---
name: portfolio
description: 이 레포의 프로젝트를 포트폴리오 원고(portfolio.md)로 정리한다. 레포 구조·커밋 로그·세션 메모리에서 만든 사람 인터뷰, 처음 그린 구상, 유저 플로우, 개발 과정, 붙인 API, 시행착오를 뽑아 정해진 템플릿으로 채운다. 사용자가 "포트폴리오 정리", "이 프로젝트 포트폴리오로 뽑아줘", "/portfolio" 라고 하면 쓴다.
---
...
그래서 이 원고의 뼈대는 넷이고, 본문에서 그 차례로 선다.

- **인터뷰** — 어떤 계기로 시작했고 누가 쓰는지. 사람 목소리
- **구상** — 그때 머릿속에 그린 프로토타입을 리스트로. 판단이 아니라 그림
- **개발 과정** — 생각이 흘러간 순서. 막힌 지점, 그때 내린 판단, 붙인 것
- **시행착오** — 그중에서도 유난히 헤맸던 개별 사건의 부검

**`## 기획` 섹션은 없앴다.** 라벨을 여덟까지 늘려 봤지만 결국 "무엇을 뺐는지"를
해명하는 표가 됐다. 왜 만들었는지는 인터뷰가, 무엇을 만들려 했는지는 구상이,
어디까지 하고 무엇을 접었는지는 개발 과정이 이야기로 말하는 편이 낫다.
**혼자 만든 물건에 착수 전 기획서는 없었다** — 없던 문서를 라벨로 지어내지 않는다.
옛 원고에 `## 기획` 이 남아 있으면 렌더러가 그리기는 하지만, 새로 쓰지 않는다.

나머지 섹션은 이 넷을 뒷받침하는 자료다.
...
3. **사용자에게 묻는다** — 왜 만들었는지, 누가 쓰는지, 처음에 뭘 그렸는지는 코드에
   안 남는다. 레포에서 못 캐면 **그냥 물어본다.** `## 인터뷰` 의 라벨 표(아래)를
   질문지로 쓴다. 지어내지 않는다.
...
블로그가 **같은 질문으로 뜨는 라벨끼리** 하나로 묶어 값만 아래로 쌓아 그린다
(`문제`·`동기`·`계기` 는 한 질문이다). 억지로 한 줄에 몰아넣지 않는다.
다만 셋을 넘기면 한 문답이 길어지니, 겹치는 줄은 합치는 편이 낫다.
...
`## 기획` 을 없애면서 **`사용자` 가 여기로 돌아왔다** — 누가 쓰는지는 만든 사람에게
묻는 말이다. 옛 원고의 `**문제**` 줄도 그대로 두면 된다(`계기` 와 같은 질문으로 뜬다).
다만 `범위`·`제약`·`정책` 처럼 결정을 적는 라벨은 여기 쓰지 않는다 — 그건 문답이 아니라
판단이라 `## 개발 과정` 에서 이야기로 나와야 한다.
...
- **배운 것·반응** — 만들고 나서의 이야기. 다른 섹션에는 없는 자리라 여기서만 나온다
...
`## 인터뷰` 바로 뒤에 온다. **왜 만들었는지를 사람 목소리로 들은 다음, 그때 머릿속에
있던 그림을 보는 순서다.** 기능 목록이 먼저 서면 "왜 이걸 만들었나"가 안 나온다.

**여기는 판단하는 자리가 아니다.** 머릿속에 대략 그린 프로토타입을 리스트로 옮기는
수준이면 된다. 어디까지 하기로 했고 무엇을 조건으로 뒀는지 같은 판단은 `## 개발 과정`
이 이야기로 말한다. 층을 섞지 않는다.

**이건 누가 시켜서 받은 스펙이 아니다.** 대개 "이런 게 있으면 좋겠는데" 하고 혼자
시작한 것이고, 그때 머릿속에 있던 건 프로토타입 한 덩어리다. 그 얇은 상태를 얇은
그대로 적는다. 그럴듯하게 부풀리지 않는다.

**착수 시점에서 얼린다.** 원고를 쓰는 지금은 다 만들어 본 뒤라 무엇이 필요했는지
...
**만들기 전후에 내린 판단이 전부 여기 자리다.** 어디까지만 하기로 했는지, 무엇을
조건으로 뒀는지, 저울질하다 무엇으로 갈아탔는지 — 원고에 `## 기획` 섹션이 없는 이유다.
라벨 한 줄로 세우면 결론만 남고, 언제 왜 그랬는지가 빠진다. "처음엔 검색으로
...
- **만든 의도와 사용자는 코드에 없다.** 커밋에서 못 읽으면 사용자에게 묻는다
...
- 인터뷰에 넣은 라벨과, 근거가 없어 뺀 것
- 사용자에게 물어야 채워지는 것 (만든 의도 등)
