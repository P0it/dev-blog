...
4. **화면을 찍는다** — 웹 앱이면 `capture-screens.mjs` 로 직접 찍는다.
   사용자에게 스크린샷을 달라고 하기 전에 이걸 먼저 해본다. 자세한 건 아래 "화면을 고르는 법".
5. **초안을 쓴다** — 아래 템플릿을 그대로 채운다.
6. **`portfolio.md` 로 저장한다.**
...
**파일** ./screens/list.png
**설명** 발행한 글과 초안이 한 자리에 섞여 있어요

### 초안 요청

**파일** ./screens/request.png
**설명** URL 을 넣는 칸 하나가 전부입니다
...
그래서 **본문의 첫 섹션은 `## 기획`** 이 된다 — 이 물건이 뭔지는 표지에서 끝내고,
...
**정지 화면 몇 장을 그리드로 늘어놓는 자리다.** 기획을 읽고 "그래서 이게 어떻게
생겼는데?" 하는 대목에서 바로 보여준다. `### 화면 이름` + `**파일**` + `**설명**` 으로,
시연과 같은 모양으로 적는다.

- **3~5 장.** 이 물건을 설명하는 데 꼭 필요한 화면만. 전 화면을 늘어놓지 않는다
- 제목은 **화면 이름**이다. `글 목록`, `초안 요청`, `설정`. 라우트나 컴포넌트명이 아니다
- 설명은 한 문장. 화면에 안 보이는 것만 적는다. 보이는 걸 또 쓰면 군더더기다
...
**사용자에게 스크린샷을 달라고 하지 마라.** 웹 앱이면 이 스킬에 딸린 스크립트가
앱을 띄우고 라우트를 돌며 찍는다. 로고는 레포에 있지만 화면은 없으니, 이게 유일한 소스다.

```
node ~/.claude/skills/portfolio/capture-screens.mjs \
  --cmd "npm run dev" --base http://localhost:3000 --out ./screens \
  /:홈 /posts:글목록 /settings:설정
```

앱을 띄우고, 뜰 때까지 기다리고, 찍고, 서버를 정리한 뒤 `## 화면` 블록을 찍어 준다.
그 블록을 원고에 붙이고 **`**설명**` 줄만 채운다** — 화면만 보고는 알 수 없는 내용이라
그건 스크립트가 못 한다.

**파일은 원고 옆에 둔다.** 결과는 이 레포 루트에 이렇게 떨어진다.
...
screens/
  01-home.png
  02-posts.png
  _section.md     ← 붙여넣기용 사본
demo/             ← 화면 녹화가 있으면
  flow.mp4
```

`**파일**` 경로는 **원고 파일 기준 상대경로**로 적는다 (`./screens/01-home.png`).
블로그 레포로 옮길 때 슬러그 폴더로 다시 묶이지만, 그건 저쪽 세션이 할 일이다.
여기서는 원고와 파일이 나란히 있기만 하면 된다. 사용자가 원고를 넘길 때
**이 폴더들도 같이 가져가야 한다**는 걸 마지막 보고에 적는다.

**하기 전에**
1. 매니페스트에서 개발 서버 명령을 확인한다 (`npm run dev`, `pnpm dev`, `bun dev` …)
2. 라우트를 뽑는다. Next.js 면 `app/`·`pages/` 구조, 그 외는 라우터 설정에서
3. **찍을 화면을 고른다.** 라우트를 전부 넘기지 않는다 — 3~5 개만
4. 앱이 켜지려면 `.env` 가 필요한지 본다. 없으면 그대로 실패한다

**옵션**
- `--mobile` 폰 뷰포트(390×844). 모바일 앱이면 이쪽
- `--full` 페이지 전체를 이어 찍는다. 기본은 첫 화면만
- `--dark` 다크 모드로
- `--wait 2000` 애니메이션·데이터 로딩이 느리면 늘린다
- `--auth ./auth.json` 로그인 뒤 화면. playwright `storageState` 파일이 필요하다
- `--base` 만 주고 `--cmd` 를 빼면 이미 떠 있는 서버에 붙는다

**자동으로 안 되는 것 — 이건 사용자에게 물어라**
- **로그인이 필요한 화면.** `--auth` 에 줄 `storageState` 를 사용자가 만들어 줘야 한다.
  못 받으면 그 화면은 빼고 보고에 적는다
- **데이터가 비어 있는 화면.** 로컬 DB 가 비었으면 빈 목록이 찍힌다. 규약상 빈 상태는
  넣지 않는 화면이다 — 시드가 있는지 물어보고, 없으면 그 화면을 뺀다
- **네이티브 모바일 앱.** 에뮬레이터 없이는 못 찍는다. Flutter 는 `flutter run -d chrome`
  으로 웹에 띄우면 `--mobile` 로 찍을 수 있다. 그것도 안 되면 사용자에게 녹화를 부탁한다
- **`## 시연` 영상.** 흐름을 따라가려면 무엇을 누를지 알아야 해서 라우트 나열로는 안 된다.
  사용자에게 폰·화면 녹화를 부탁하는 편이 빠르다
...
모바일 앱이면 폰 기본 화면 녹화로 찍어 파일만 원고 옆에 두면 된다. 따로 편집하거나
...
- 자동으로 찍은 화면 수와, 못 찍은 화면(로그인·빈 데이터·네이티브)과 그 이유
