...
- **첫 장이 목록 카드의 대표 화면이 된다.** 홈이나 메인 화면을 맨 앞에 둔다.
  그 한 장이 긴 캡처면 목록 카드 안에서도 훑어 내린다 — 카드를 열기 전에 뭐가
  들었는지 보이게. 홈이 한 화면에 다 들어오는 그림은 그냥 선다
...
- **판 모양은 `platform` 이 정한다. 그 안에서 비율은 블로그가 알아서 고른다**
  ...
  - 스크롤까지 담은 긴 캡처(전체 페이지) → 폰 판 안에서 위에서 아래로 천천히 훑어 내린다.
    길수록 천천히 움직인다. 그래서 **긴 캡처를 잘라 여러 장으로 나눌 필요가 없다**.
    상세에서는 손을 얹은 한 장만 움직인다 — 긴 캡처 여러 장이 동시에 훑어 내리면
    어디를 봐야 할지 모른다. 손가락에는 hover 가 없으니 판을 누르면 원본이 크게 열린다
