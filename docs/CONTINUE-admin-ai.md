...
## ⚠ 미커밋 (2026-05-19 UI 개선 4건 — 빌드 OK·lint 클린·스모크 200, 사용자가 "그대로 두기" 선택)

커밋/푸시 안 함. 다음에 사용자 확인 후 커밋 결정. 변경 파일:
- `src/app/admin/posts/page.tsx`, `src/app/admin/page.tsx` — "새 글"→"새 글 쓰기" + SquarePen 아이콘(Plus 교체)
- `src/components/admin/PostsList.tsx` — AiBadge pending/processing 통합(파란칩+스피너 "생성중");
  행 발행/초안 칩 제거 → draft만 제목 앞 보라 "초안" 마커, published 무표시
- `src/app/admin/settings/page.tsx` — jobChip pending/processing 통합(파란 "생성중")
- `src/components/admin/PostEditor.tsx` — metaOpen 상태 + 토파바 "메타 접기/펼치기"
  토글(접으면 메타 패널 display:none, grid 2단).
- `src/components/layout/AdminTopbar.tsx` — 어드민 전 페이지에 ThemeToggle(다크모드)
  추가(기존 `@/components/ui/ThemeToggle` 재사용, nav-right).
- 잡파일 `.claude/`는 커밋 제외. 빌드 OK·lint 클린·스모크 200.
