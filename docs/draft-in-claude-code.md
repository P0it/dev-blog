...
`category` 유효값(최상위/하위): `tech`(`claude` · `infra` · `ai`) · `business`(`insights`) · `design` · `health`.
(현재 `categories` 테이블 기준 — 새 카테고리는 `/admin/categories` 또는 직접 insert 로 추가한다.)
...
### 영상 캡처 — YouTube 글의 소제목별 장면 (기본)

YouTube 글이면 각 `## 소제목`에 영상의 대표 장면을 캡처해 넣는 것을 기본으로 한다
(영상 글의 기본 시각자료). **선별은 자동이 아니라 이 세션이 한다** — 후보 프레임을
Read 로 직접 보고 고르므로 별도 비전 API·비용이 없다. 쓸 만한 컷이 없는 소제목만
건너뛴다.
...
- `/insight <질문·생각>` — `.claude/commands/insight.md`. URL 없이 질문·주장을 받아
  웹 리서치 근거를 갖춘 의견 글을 쓴다. 장르 규약은 `INSIGHT.md`(루트, `POSTING.md` 옆).
  적재 파이프라인(`drafts/<slug>.md` → `npm run draft -- push`)은 `/draft` 와 동일.
