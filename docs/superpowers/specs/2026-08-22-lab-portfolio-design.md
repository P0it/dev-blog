...
| 섹션 | 렌더링 | 움직임 |
|---|---|---|
| 화면 | 폰 판 그리드. 판 아래 이름표 + 한 줄 설명. 영상도 같은 자리에 그린다. 판에 올리면 우상단에 확대 표시가 뜨고, 누르면 원본을 확대 창에서 본다(좌우 화살표·←→ 키로 옆 화면으로 넘어간다). 그리드의 판은 실제 화면보다 훨씬 작아 글씨가 안 읽힌다 — 원본을 보는 길이 없으면 목업 전시로 끝난다 | 카드 스태거 |
