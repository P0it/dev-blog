...
| 섹션 | 내용 | 형식 제약 |
|---|---|---|
| `## 화면` | 정지 화면 6~10 장 그리드 (폰 판 + 이름표 + 한 줄 설명) | `### 이름` + `**파일**` + `**설명**`. 짧은 영상도 같은 자리에 넣는다. 그림 비율은 렌더러가 판별한다 — 가로 화면은 두 칸, 스크롤까지 담긴 긴 캡처는 판 안에서 자동으로 훑어 내린다 |
| `## 데이터와 API` | 붙인 외부 데이터·서비스 | `### 이름` 단위. `**용도**` 한 줄 필수, `방식`·`갱신` 은 칩, 나머지 라벨은 펼침. 코드 레벨 서술 금지 |
...
| 섹션 | 렌더링 | 움직임 |
|---|---|---|
| 화면 | 폰 판 그리드. 판 아래 이름표 + 한 줄 설명. 영상도 같은 자리에 그린다 | 카드 스태거 |
| 기술 스택 | 브랜드 아이콘 + 이름 칩 나열 | 진입 시 페이드업 |
| 데이터와 API | 3열 압축 카드. 브랜드 로고 + 이름 + 용도 한 줄 + 칩, 나머지는 "자세히" 로 펼침 | 카드 스태거 |
