...
| 데이터와 API | 3열 압축 카드. 브랜드 로고 + 이름 + 용도 한 줄 + 링크까지만. 카드 전체가 누를 자리(링크만 예외)이고, 나머지 라벨은 **확대 창**에서 라벨-값으로 읽는다. **칩과 제자리 펼침은 접었다** — ①칩에 넣던 `방식`·`갱신` 값이 죄다 한 문장 이상이라 잘려서 아무 정보도 주지 못했고 ②그리드 한 줄의 높이는 가장 큰 칸이 정하므로 카드 하나를 펼치면 옆 카드가 같이 늘어나 빈 상자가 됐다 | 카드 스태거, 올리면 판이 살짝 뜬다 |
...
**확대 창은 셋이 한 컴포넌트를 쓴다** — 구조도·데이터와 API·화면 모두 `LabModal`
(`src/components/project/LabModal.tsx`). 화면을 덮는 판은 반드시 `body` 로 빼서 그린다.
