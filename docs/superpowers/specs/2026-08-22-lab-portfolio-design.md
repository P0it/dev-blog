...
| 섹션 | 내용 | 형식 제약 |
|---|---|---|
| `## 화면` | 정지 화면 6~10 장 그리드 (판 + 이름표 + 한 줄 설명) | `### 이름` + `**파일**` + `**설명**`. 짧은 영상도 같은 자리에 넣는다. **판 모양은 프런트매터 `platform` 이 정하고**(아래 2-1), 그 안에서 그림 비율은 렌더러가 판별한다 — 폰 판에서 가로 화면은 두 칸, 스크롤까지 담긴 긴 캡처는 판 안에서 훑어 내린다(상세는 손 얹은 한 장만, 목록 카드는 그냥). 판을 누르면 원본을 확대 창에서 본다 |
