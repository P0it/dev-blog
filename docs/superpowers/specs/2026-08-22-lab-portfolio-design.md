...
| `## 구조` | 계층과 그 사이로 오가는 것 | mermaid 블록 1개(subgraph 로 경계·라벨 붙은 화살표) + 노드·상자 이름과 같은 `### 제목` 설명 |
...
| 구조 | 다이어그램을 본문 폭 판에 크게(`fontSize` 16, 확대 18). **제 크기보다 크게는 안 그린다** — mermaid 가 svg 인라인 style 에 남긴 자연 폭을 판(figure)의 `max-width` 로 올려 달아, 작은 그림은 원래 크기에서 멈추고 가운데 선다. 선 색은 카드 테두리(`--lab-line`)가 아니라 다이어그램 전용 토큰(`--lab-diag-node`·`--lab-diag-edge`·`--lab-diag-cluster`)을 쓴다. 10% 짜리 테두리색을 쓰면 화살표가 배경에 묻힌다. 판 머리줄에 "크게 보기", 아래에 범례(실선=보내는 것, 점선=되돌아오는 것)와 계층 이름. 설명은 2열 카드, 제목 옆에 소속 덩어리 칩 | **포인터**로 잇는다 — 노드에 올리면 그 설명이 노드 옆 툴팁으로, 설명 카드에 올리면 같은 이름의 노드·상자가 점등. 스크롤 동기화와 sticky 는 쓰지 않는다 |
