...
| 섹션 | 내용 | 형식 제약 |
|---|---|---|
| `## 제품 소개` (본문 아님 — 표지 아래 리드) | 뭐 하는 물건인지, 처음 보는 사람 기준 | 2~3 문단, 기능 나열 금지 |
...
| 구조 | 다이어그램을 본문 폭 판에 크게(`fontSize` 16, 확대 18). 판 머리줄에 "크게 보기", 아래에 범례(실선=보내는 것, 점선=되돌아오는 것)와 계층 이름. 설명은 2열 카드, 제목 옆에 소속 덩어리 칩 | **포인터**로 잇는다 — 노드에 올리면 그 설명이 노드 옆 툴팁으로, 설명 카드에 올리면 같은 이름의 노드·상자가 점등. 스크롤 동기화와 sticky 는 쓰지 않는다 |
