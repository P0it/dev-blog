...
| `## 화면` | 정지 화면 3~5 장 그리드 (폰 판 + 이름표 + 한 줄 설명) | `### 이름` + `**파일**` + `**설명**` |
...
| 구조 | 다이어그램을 패널에 크게 | 다이어그램 sticky 고정, 옆 설명을 스크롤하면 **같은 이름의** 노드나 subgraph 상자가 점등 (이름을 못 찾으면 N 번째 노드로 폴백). 구조는 순서가 아니라 배치라 번호를 붙이지 않는다 |
...
// 프런트매터 stack 을 받아 `## 기술 스택` 섹션을 구상 뒤에 끼운 최종 목록.
